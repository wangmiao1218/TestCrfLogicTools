...
public class CrfLogic_bak {
	...
	*/
	public static void queryCrfdataByPatAndWriteResults(Excel excel,String mongodbIp) throws JSONException{
		...
		//将crfdata结果写入excel
		CrfLogic_bak.writeCrfdataIntoExcel(excel, rowNumAndcrfdataMap);
		...
		*/		
		...
		if ("success".equals(callableTest) && "success".equals(callableDevelop)) {
			...
			String callableAuto = new Callable<String>() {
				@Override
				public String call() throws Exception {
					CrfLogic_bak.requestCrfAutoInterfaceByPat(cellNumAndPatMap, httpUrl, disease);
					return "success";
				}
			}.call();;
			//将pat写入excel
			String callableExcel = new Callable<String>() {
				@Override
				public String call() throws Exception {
					CrfLogic_bak.writePatIntoExcel(excel, cellNumAndPatMap);
					return "success";
				}
			}.call();;
